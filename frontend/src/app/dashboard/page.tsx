        ...
        <main className="flex-1 p-4 sm:p-6 lg:p-8 max-w-6xl mx-auto w-full">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
            {stats.map((item, idx) => {
              ...
              return (
                <Card key={idx} className="border-[#E5E7EB] bg-white shadow-sm">
                  <CardContent className="p-5 flex items-center justify-between">
                    <div>
                      <span className="text-xs font-semibold text-slate-500">{item.title}</span>
                      <div className="text-2xl font-extrabold text-[#1F2937] mt-1">{item.value}</div>
                      <span className="text-[11px] text-slate-400 mt-0.5 block font-medium">{item.label}</span>
                    </div>
